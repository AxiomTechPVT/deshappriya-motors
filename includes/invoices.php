...
function handle_invoice_request(string $section): void
{
    ...
    if ($section==='invoices' || $section==='invoices-mine' || $section==='invoices-quick') {
        ...
        $filters=['search'=>trim((string)($_GET['search']??'')),'type'=>$_GET['type']??'','status'=>$_GET['status']??'','payment_method'=>is_string($_GET['payment_method']??null)?$_GET['payment_method']:'','date_from'=>trim((string)($_GET['date_from']??'')),'date_to'=>trim((string)($_GET['date_to']??'')),'created_by'=>$section==='invoices-mine'?(int)(current_user()['id']??0):0];
