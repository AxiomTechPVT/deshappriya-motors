<?php $user = current_user(); $isAdmin = $user['role'] === 'administrator'; $pendingCashHandoverCount = 0; $pendingAdvanceRequestCount = 0; if ($isAdmin) { try { require_once __DIR__ . '/cashier-advances.php'; ensure_cashier_advance_table(); $pendingCashHandoverCount = (int) database()->query("SELECT COUNT(*) FROM cashier_registers WHERE handover_status = 'pending'")->fetchColumn(); $pendingAdvanceRequestCount = (int) database()->query("SELECT COUNT(*) FROM cashier_advance_requests WHERE status = 'pending'")->fetchColumn(); } catch (Throwable $ignored) {} } ?>
<!doctype html>
<html lang="en">
<head>
    <meta charset="utf-8">
...
<div class="app-shell <?= $referenceLayout ? 'customer-layout' : '' ?> <?= ($section ?? '') === 'vehicles' ? 'vehicle-list-page' : '' ?>">
    ...
    <div class="main-area">
        <header class="topbar <?= $isAdmin ? 'admin-topbar' : '' ?>">
            ...
            <div class="topbar-tools"><span class="header-date d-none d-md-inline"><?= date('D, d M Y') ?></span><?php if ($isAdmin): ?><a class="notification-dot <?= ($pendingCashHandoverCount + $pendingAdvanceRequestCount) > 0 ? 'notification-active' : '' ?>" href="index.php?page=admin&amp;section=cashier-advance-requests" aria-label="Notifications" title="Advance requests and cash handovers"><?= ($pendingCashHandoverCount + $pendingAdvanceRequestCount) > 0 ? ($pendingCashHandoverCount + $pendingAdvanceRequestCount) : '!' ?></a><?php else: ?><span class="notification-dot" aria-label="Notifications">!</span><?php endif; ?><div class="user-chip"><div class="avatar"><?= e(strtoupper(substr($user['name'], 0, 1))) ?></div><div class="d-none d-sm-block"><strong><?= e($user['name']) ?></strong><small><?= e(ucfirst($user['role'])) ?></small></div></div></div>
        </header>
