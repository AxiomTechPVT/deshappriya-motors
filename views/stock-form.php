            ...
            <form data-stock-save-form method="post" action="index.php?page=admin&amp;section=<?= e($section) ?>&amp;id=<?= (int)$id ?>">
                ...
                <div class="estimate-form-section">
                    ...
                    <div class="row g-3">
                        ...
                        <?php if (!$isEdit): ?>
                        <div class="col-6"><label class="form-label">Purchase Date</label><input class="form-control" type="date" name="purchase_date" value="<?= e($input['purchase_date'] ?? date('Y-m-d')) ?>"></div>
                        <div class="col-6"><label class="form-label">Supplier Invoice / Bill No.</label><input class="form-control" name="invoice_no" value="<?= e($input['invoice_no'] ?? '') ?>"></div>
                        <div class="col-6"><label class="form-label">Payment Status</label><select class="form-select" name="payment_status" id="new-stock-payment-status"><option value="paid" <?= ($input['payment_status'] ?? 'paid') === 'paid' ? 'selected' : '' ?>>Fully Paid</option><option value="partial" <?= ($input['payment_status'] ?? '') === 'partial' ? 'selected' : '' ?>>Partial</option><option value="due" <?= ($input['payment_status'] ?? '') === 'due' ? 'selected' : '' ?>>Due</option></select></div>
                        <div class="col-6"><label class="form-label">Payment Method</label><select class="form-select" name="payment_method"><option value="cash" <?= ($input['payment_method'] ?? 'cash') === 'cash' ? 'selected' : '' ?>>Cash</option><option value="card" <?= ($input['payment_method'] ?? '') === 'card' ? 'selected' : '' ?>>Card</option><option value="bank_transfer" <?= ($input['payment_method'] ?? '') === 'bank_transfer' ? 'selected' : '' ?>>Online Transfer</option><option value="cheque" <?= ($input['payment_method'] ?? '') === 'cheque' ? 'selected' : '' ?>>Cheque</option></select></div>
                        <div class="col-6" id="new-stock-paid-field"><label class="form-label">Amount Paid (Rs.)</label><input class="form-control" type="number" min="0" step="0.01" name="paid_amount" value="<?= e($input['paid_amount'] ?? '0') ?>"><small class="bay-field-hint">For partial payments; fully paid uses the purchase total.</small></div>
                        <?php endif; ?>
...
<?php if (!$isEdit): ?><script>
(() => {
    const status = document.getElementById('new-stock-payment-status');
    const paidField = document.getElementById('new-stock-paid-field');
    if (!status || !paidField) return;
    const update = () => { paidField.hidden = status.value !== 'partial'; };
