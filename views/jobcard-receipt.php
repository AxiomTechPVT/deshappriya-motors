<?php $paymentAllocation = job_payment_allocation((float)$payment['subtotal'], (float)$payment['service_charge'], (float)$payment['paid_amount'], (float)$payment['amount']); ?>
...
<section class="admin-panel job-receipt-card thermal-receipt">
    ...
    <div class="thermal-total-lines"><div><span>Subtotal</span><strong>Rs. <?= number_format((float)$payment['subtotal'] + (float)$payment['service_charge'], 2) ?></strong></div><div><span>Discount<?= ($payment['discount_type'] ?? '') === 'percentage' ? ' (' . number_format((float)$payment['discount_value'],2) . '%)' : '' ?></span><strong>- Rs. <?= number_format((float)($payment['discount'] ?? 0),2) ?></strong></div><?php $partReduction = array_sum(array_map('item_price_reduction', $receiptItems)); if ($partReduction > 0): ?><div><span>Part price reductions (included above)</span><strong>Rs. <?= number_format($partReduction,2) ?></strong></div><?php endif; ?><div class="thermal-grand-total"><span>FULL TOTAL</span><strong>Rs. <?= number_format((float)$payment['total_amount'], 2) ?></strong></div></div>
    <div class="thermal-payment-lines"><div><span>Payment Method</span><strong><?= e(payment_method_label($payment['payment_method'])) ?></strong></div><div><span>Amount Received</span><strong>Rs. <?= number_format((float)($payment['amount_received'] ?: $payment['amount']), 2) ?></strong></div><div><span>Parts Covered From This Payment</span><strong>Rs. <?= number_format($paymentAllocation['current_parts'], 2) ?></strong></div><div><span>Service Charge From This Payment</span><strong>Rs. <?= number_format($paymentAllocation['current_service'], 2) ?></strong></div><div><span>Change Given</span><strong>Rs. <?= number_format((float)($payment['change_amount'] ?? 0), 2) ?></strong></div><div><span>Parts Balance</span><strong>Rs. <?= number_format($paymentAllocation['parts_balance'], 2) ?></strong></div><div><span>Service Charge Balance</span><strong>Rs. <?= number_format($paymentAllocation['service_balance'], 2) ?></strong></div><div><span>Total Balance Due</span><strong>Rs. <?= number_format((float)$payment['balance_amount'], 2) ?></strong></div></div>
