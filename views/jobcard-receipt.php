...
<section class="admin-panel job-receipt-card thermal-receipt">
    <div class="receipt-brand">
        ...
        <small><?= e($receiptSettings['contact_number'] ?? '') ?></small>
    </div>
    ...
    <div class="thermal-payment-lines"><div><span>Payment Method</span><strong><?= e(ucfirst($payment['payment_method'])) ?></strong></div><div><span>Amount Received</span><strong>Rs. <?= number_format((float)($payment['amount_received'] ?: $payment['amount']), 2) ?></strong></div><div><span>Change Given</span><strong>Rs. <?= number_format((float)($payment['change_amount'] ?? 0), 2) ?></strong></div><div><span>Balance Due</span><strong>Rs. <?= number_format((float)$payment['balance_amount'], 2) ?></strong></div></div>
