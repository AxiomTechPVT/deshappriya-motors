document.addEventListener('DOMContentLoaded', () => {
        ...
        if (cardId && amountBox) {
            fetch('index.php?page=admin&section=jobcards-payment-summary&id=' + encodeURIComponent(cardId)).then((response) => response.json()).then((summary) => {
                ...
                if (paymentButton) paymentButton.addEventListener('click', () => {
                    ...
                    let payable = due > 0.009 ? due : fullTotal;
                    const beforeDiscount = fullTotal + Number(summary.discount || 0);
                    const alreadyPaid = Number(summary.paid || 0);
                    if (payable <= 0.009) {
                    ...
                    backdrop.innerHTML = '<section class="job-start-modal payment-modal"><div class="manual-part-modal-heading"><div><span class="modal-eyebrow">PAYMENT</span><h2>Collect Payment</h2><p>Enter work times and the amount received before generating the bill.</p></div><button type="button" class="customer-modal-close" data-payment-close>&times;</button></div><form method="post" action="index.php?page=admin&section=jobcards-payment"><input type="hidden" name="csrf_token" value="' + escapeHtml(csrf) + '"><input type="hidden" name="job_card_id" value="' + escapeHtml(cardId) + '"><div class="payment-due-card"><span>Full Job Amount (after discount)</span><strong data-discount-total>Rs. ' + fullTotal.toFixed(2) + '</strong><small data-discount-outstanding>Outstanding: Rs. ' + due.toFixed(2) + '</small></div><div class="payment-discount-controls"><label>Discount (Optional)<select class="form-select" name="discount_type" data-job-discount-type><option value="">No Discount</option><option value="percentage">Percentage (%)</option><option value="fixed">Amount (Rs.)</option></select></label><label>Discount Value<input class="form-control" type="number" name="discount_value" min="0" step="0.01" value="0" data-job-discount-value></label><small>Discount: <strong data-job-discount-amount>Rs. 0.00</strong></small></div><div class="performance-time-grid"><label>Start Time *<input class="form-control" type="datetime-local" name="performance_start" value="' + escapeHtml(savedStart) + '" required></label><label>End Time *<input class="form-control" type="datetime-local" name="performance_end" value="' + escapeHtml(savedEnd) + '" required></label></div><label>Amount Received (Rs.) *<input class="form-control payment-received" type="number" name="amount" value="' + startingAmount.toFixed(2) + '" min="0.01" step="0.01" required></label><label>Payment Method<select class="form-select payment-method" name="payment_method"><option value="cash">Cash</option><option value="card">Card</option><option value="bank">Online Transfer</option><option value="other">Other</option></select></label><div class="payment-calculation"><div><span>Applied Amount</span><strong data-payment-applied>Rs. ' + payable.toFixed(2) + '</strong></div><div><span>Change / Balance</span><strong data-payment-change>Rs. 0.00</strong></div></div><div class="job-start-modal-actions"><button type="button" class="btn btn-light" data-payment-close>Close</button><button class="btn btn-primary" type="submit">Pay &amp; Generate Receipt</button></div></form></section>';
                    document.body.appendChild(backdrop);
                    const paymentForm = backdrop.querySelector('form');
                    let paymentSubmitting = false;
                    paymentForm.addEventListener('submit', (event) => {
                        if (paymentSubmitting) { event.preventDefault(); return; }
                    ...
                    discountValue.value = summary.discount_value ?? summary.discount ?? 0;
                    const discountLocked = alreadyPaid > 0 || summary.status === 'completed';
                    discountType.disabled = discountLocked;
                    const updatePayment = (discountChanged = false) => {
                        const previousPayable = payable;
                        const raw = Math.max(0, Number(discountValue.value) || 0);
                        discountValue.disabled = discountLocked || !discountType.value;
                        discountValue.max = discountType.value === 'percentage' ? '100' : beforeDiscount.toFixed(2);
                        const discount = discountType.value === 'percentage' ? Math.round(beforeDiscount * Math.min(100,raw)) / 100 : discountType.value === 'fixed' ? Math.min(beforeDiscount,raw) : 0;
                        const total = Math.max(0, Math.round((beforeDiscount-discount)*100)/100);
                        payable = Math.max(0, Math.round((total-alreadyPaid)*100)/100);
                        if (discountChanged && Math.abs(Number(received.value)-previousPayable) < 0.009) received.value = payable.toFixed(2);
                        received.min = payable <= 0.009 ? '0' : '0.01';
                        if (method.value === 'cash') received.removeAttribute('max'); else received.max = payable.toFixed(2);
                        backdrop.querySelector('[data-discount-total]').textContent = 'Rs. ' + total.toFixed(2);
                        backdrop.querySelector('[data-discount-outstanding]').textContent = 'Outstanding: Rs. ' + payable.toFixed(2);
                        backdrop.querySelector('[data-job-discount-amount]').textContent = 'Rs. ' + discount.toFixed(2);
                        const value = Math.max(0, Number(received.value) || 0);
                        const appliedValue = Math.min(value,payable), difference = value-payable;
    ...
    if (invoiceForm && invoiceLookups) {
        ...
        // Invoice rows and totals are handled by invoices.js.
